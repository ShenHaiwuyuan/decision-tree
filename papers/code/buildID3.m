function tree = ID3(train_features,train_targets,discrete_dim,layer,varargin)
    ...
    for i=1:feaNum
        ...
        if (discrete_dim(i)) %离散特征   
            ...
            GainA= (Inode-sum(rocle.*InfoDj)); %信息增益率      
        else
            %连续属性离散化
...
%             end
            for j=1:subNum
                node(:, 1) = hist(sorted_targets(find(sorted_data <= sorted_data(j))) , Label);
                node(:, 2) = hist(sorted_targets(find(sorted_data > sorted_data(j))) , Label);
                Ps=sum(node)/L; %|Dv|/|D|
                Ln=sum(node);
                Ln=Ln+eps*(Ln==0);
                %EntD(+)
    ...
    if (discrete_dim(feature_tosplit))  %如果当前选择的这个作为分裂特征的特征是个离散特征   
        for i = 1:subNum   %遍历这个特征下无重复的特征值的数目  
            ...
            tree.child(i)= buildID3(train_features(dims, indx), train_targets(indx), discrete_dim(dims), layer, pruning);%递归  
        ...
        if ~(isempty(indx1) || isempty(indx2))  %如果<=分裂值 >分裂值的样本数目都不等于0    
            tree.child(1)= buildID3(train_features(dims, indx1), train_targets(indx1), discrete_dim(dims),layer+1);
            tree.child(2)= buildID3(train_features(dims, indx2), train_targets(indx2), discrete_dim(dims),layer+1);   
        else    
