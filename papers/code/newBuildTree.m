...
for i = 1:fea %遍历每个特征    
    ...
    if (discrete_dim(i)) %离散特征   
        ...
        info= sum(-node.*LOG2F(eps+node));  %每个特征分别计算信息熵,eps是为了防止对数为1 
        el(i) = (Inode-sum(rocle.*info))/(-sum(rocle.*LOG2F(eps+rocle))); %信息增益率  
    else
        %连续特征
        ...
        %计算分裂信息度量  
         ...
         for j = 1:nu-1  %特征i有Nbins个连续值，设定Nbins-1个可能的分割点，对每个分割点计算信息增益率
             ...
             P1= repmat(rocle, length(ale), 1); 
             P1= P1 + eps*(P1==0);    
             info= sum(-node./P1.*LOG2F(eps+node./P1)); %信息增益
             I(j)= Inode - sum(info.*Ps);   
             spl(j) =I(j)/(-sum(Ps.*LOG2F(eps+Ps)));  %第j个分割点的信息增益率
         end  
...
if (discrete_dim(feature_tosplit))  %如果当前选择的这个作为分裂特征的特征是个离散特征   
    for i = 1:nu   %遍历这个特征下无重复的特征值的数目  
        ...
        tree.child(i)= newBuildTree(train_features(dims, indices), train_targets(indices), discrete_dim(dims), layer, pruning);%递归  
