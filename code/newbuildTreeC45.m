function tree = newbuildTreeC45(train_features,train_targets,discrete_dim,layer,varargin)
    ...
    for i=1:feaNum
        ...
        if (discrete_dim(i)) %离散特征   
            ...
            for j=1:subNum-1
                ...
                Ln=Ln+eps*(Ln==0);
                %EntD(+)
                InfoDj(1)=-sum(node(:,1)./Ln(1).*(eps+node(:,1)/Ln(1)-1)/log(2));
                %EntD(-)
                InfoDj(2)=-sum(node(:,2)./Ln(1).*(eps+node(:,2)/Ln(2)-1)/log(2));
...
%                 SplitInfoA=-sum(Ps.*log2(Ps));
    ...
    if (discrete_dim(feature_tosplit))  %如果当前选择的这个作为分裂特征的特征是个离散特征   
        for i = 1:subNum   %遍历这个特征下无重复的特征值的数目  
            ...
            tree.child(i)= newbuildTreeC45(train_features(dims, indx), train_targets(indx), discrete_dim(dims), layer, pruning);%递归  
        ...
        if ~(isempty(indx1) || isempty(indx2))  %如果<=分裂值 >分裂值的样本数目都不等于0    
            tree.child(1)= newbuildTreeC45(train_features(dims, indx1), train_targets(indx1), discrete_dim(dims),layer+1);
            tree.child(2)= newbuildTreeC45(train_features(dims, indx2), train_targets(indx2), discrete_dim(dims),layer+1);   
        else    
